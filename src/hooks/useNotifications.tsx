...
export function useNotifications() {
    ...
    const removeNotification = useCallback((itemId: number, vehicleId: number, type: string) => {
        setAllNotifications(prev => prev.filter(n => !(n.itemId === itemId && n.vehicleId === vehicleId && n.type === type)));
    }, []);
    ...
    const markAsRead = useCallback(async (notif: Notification) => {
        ...
        try {
            ...
            setAllNotifications(prev => prev.map(n => n.id === notif.id ? { ...n, seen: true } : n));
        } catch (err) {
            console.error(err);
        }
    }, []);
    ...
    const getByVehicle = useCallback((vehicleId: number) => allNotifications.filter(n => n.vehicleId === vehicleId), [allNotifications]);
