...
export async function PUT(req: Request) {
  try {
    ...
    const filteredData: Partial<MyData> = {};
    Object.entries(data).forEach(([key, value]) => {
      if (value !== undefined && value !== null) {
        filteredData[key] = value as MyData[string];
      }
    });

    // 🔹 Conversion des dates si présentes
    if (filteredData.dateEntretien)
      filteredData.dateEntretien = new Date(filteredData.dateEntretien as string);
    if (filteredData.prochaineRevision)
      filteredData.prochaineRevision = new Date(filteredData.prochaineRevision as string);
    if (filteredData.ctValidite)
      filteredData.ctValidite = new Date(filteredData.ctValidite as string);
