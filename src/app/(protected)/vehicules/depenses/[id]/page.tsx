...
export default function VehicleDepensesPage() {
  ...
  const depensesForVehicle = depenses.filter((d) => d.vehiculeId === Number(id));

  if (!vehicle) {
    return <div className="p-10 text-center text-red-600 text-lg">❌ Véhicule introuvable</div>;
  }

  return <DetailDepensesForVehicle vehicle={vehicle} depenses={depensesForVehicle} />;
}
