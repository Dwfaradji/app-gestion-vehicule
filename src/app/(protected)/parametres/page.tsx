...
export default function ParametresPage() {
    ...
    const handleConfirm = () => {
        ...
        switch (type) {
            case "valider-vehicule": addVehicule(target); break;
            case "supprimer-vehicule": deleteVehicule(target.id); break;
            case "valider-email": addEmail(target.adresse); break;
            case "supprimer-email": deleteEmail(target.id); break;
            case "valider-entretien": addParametreEntretien(target); break;
            case "modifier-entretien": updateParametreEntretien(target); break;
            case "supprimer-entretien": deleteParametreEntretien(target.id); break;
            case "valider-utilisateur": addUtilisateur(target); break;
            case "supprimer-utilisateur": deleteUtilisateur(target.id); break;
            case "ajouter-conducteur": addConducteur(target); break;
            case "supprimer-conducteur": deleteConducteur(target.id); break;
            ...
            <main className="flex-1 rounded-xl shadow border border-gray-200 p-6 overflow-auto">
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        {activeTab === "Paramètres entretien" && <TabEntretien parametresEntretien={parametresEntretien} setConfirmAction={setConfirmAction} />}
