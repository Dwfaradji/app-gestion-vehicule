...
const Dashboard = () => {
    ...
    // ✅ Gère le dark mode selon les préférences système
    useEffect(() => {
    ...
    // ✅ Rafraîchit les données à l’arrivée
    useEffect(() => {
        refreshVehicules();
        refreshAll();
        refreshEmails();
    ...
    // ✅ Cartes configurées via useMemo pour éviter les recalculs inutiles
    const cards = useMemo(
        () => [
            {
                title: "Véhicules",
    ...
    return (
        ...
        >
          ...
          text-gray-800 dark:text-gray-100 shadow-md hover:shadow-lg"
                >
                    {darkMode ? (
                        <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 w-11/12 max-w-6xl mx-auto mt-12 pb-16">
                {cards.map((card, i) => (
                    <motion.div
                        ...
                        className={`cursor-pointer rounded-2xl shadow-lg relative overflow-hidden transition-all
            bg-gradient-to-r ${card.color} text-white p-6 group`}
                    >
                        {/* Effet Glow subtil */}
