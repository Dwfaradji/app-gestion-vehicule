...
export default function StatistiquesTrajetsPage() {
    ...
    const chartData = useMemo(() => {
        const grouped: Record<string, { trajets: number; km: number; carburant: number; duree: number }> = {};
    ...
    const totalKm = filteredTrajets.reduce((acc, t) => acc + ((t.kmArrivee ?? 0) - (t.kmDepart ?? 0)), 0);
    ...
    return (
        <div className="p-8 bg-gray-50 min-h-screen space-y-8">
            ...
            <div className="flex flex-wrap gap-2">
                <ArchiveButton endpoint="/api/archive?type=vehicules" filename="vehicules.pdf" label="Exporter les véhicules" />
                <ArchiveButton endpoint="/api/archive?type=trajets" filename="trajets.pdf" label="Exporter les trajets" />
                <ArchiveButton endpoint="/api/archive?type=depenses" filename="depenses.pdf" label="Exporter les dépenses" />
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 bg-white p-4 rounded-2xl shadow border border-gray-200">
                <select value={periode} onChange={e => setPeriode(e.target.value as "jour"|"mois"|"année")} className="border border-gray-300 rounded-md px-3 py-2 bg-gray-50">
                    <option value="jour">Jour</option>
                    <option value="mois">Mois</option>
                    <option value="année">Année</option>
                </select>
                <select value={selectedConducteur} onChange={e => setSelectedConducteur(e.target.value === "all" ? "all" : Number(e.target.value))} className="border border-gray-300 rounded-md px-3 py-2 bg-gray-50">
                    <option value="all">Tous les conducteurs</option>
                    {conducteurs.map(c => <option key={c.id} value={c.id}>{c.prenom} {c.nom}</option>)}
                </select>
                <select value={selectedVehicule} onChange={e => setSelectedVehicule(e.target.value === "all" ? "all" : Number(e.target.value))} className="border border-gray-300 rounded-md px-3 py-2 bg-gray-50">
                    <option value="all">Tous les véhicules</option>
                    {vehicules.map(v => <option key={v.id} value={v.id}>{v.modele} ({v.immat})</option>)}
                </select>
                <button onClick={() => setChartType(chartType === "line" ? "bar" : "line")} className="flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition">
                    {chartType === "line" ? <><BarChart3 size={18}/> Mode Barres</> : <><LineChartIcon size={18}/> Mode Linéaire</>}
                </button>
                <div className="flex flex-wrap gap-2 ml-auto">
                    {metrics.map(m => (
                        <button key={m.key} onClick={() => setVisibleMetrics(prev => ({...prev,[m.key]:!prev[m.key]}))} className={clsx("px-3 py-1 rounded-full text-sm font-medium border transition", visibleMetrics[m.key] ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 hover:bg-gray-100 border-gray-300")}>
                            {m.label}
                        </button>
                    ))}
                </div>
                <button onClick={handleExportPDF} className="flex items-center gap-2 px-4 py-2 bg-green-100 text-green-700 rounded-md hover:bg-green-200 transition">
            ...
            <div ref={exportRef} className="space-y-8">
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                        {chartType === "line" ? (
                            <LineChart data={chartData}>
                                ...
                                <Tooltip />
                                <Legend />
                                {visibleMetrics.trajets && <Line dataKey="trajets" stroke={colors.trajets} name="Trajets" />}
                                {visibleMetrics.km && <Line dataKey="km" stroke={colors.km} name="Km" />}
                                {visibleMetrics.carburant && <Line dataKey="carburant" stroke={colors.carburant} name="Carburant (L)" />}
                                {visibleMetrics.duree && <Line dataKey="duree" stroke={colors.duree} name="Durée (h)" />}
                            </LineChart>
                        ) : (
                            <BarChart data={chartData}>
                                ...
                                <Tooltip />
                                <Legend />
                                {visibleMetrics.trajets && <Bar dataKey="trajets" fill={colors.trajets} name="Trajets" />}
                                {visibleMetrics.km && <Bar dataKey="km" fill={colors.km} name="Km" />}
                                {visibleMetrics.carburant && <Bar dataKey="carburant" fill={colors.carburant} name="Carburant (L)" />}
                                {visibleMetrics.duree && <Bar dataKey="duree" fill={colors.duree} name="Durée (h)" />}
                            </BarChart>
                        )}
                    </ResponsiveContainer>
                </div>
                ...
                {/* Table */}
                <div className="bg-white p-6 rounded-2xl shadow border border-gray-200">
                    <h2 className="text-lg font-semibold mb-4 text-gray-700">Trajets complets ({filteredTrajets.length})</h2>
                    <Table data={filteredTrajets} columns={[
                        { key: "conducteur", label: "Conducteur", render: t => { const c = conducteurs.find(c => c.id === t.conducteurId); return c ? `${c.prenom} ${c.nom}` : "-"; } },
                        { key: "vehicule", label: "Véhicule", render: t => { const v = vehicules.find(v => v.id === t.vehiculeId); return v ? `${v.modele} (${v.immat})` : "-"; } },
                        { key: "destination", label: "Destination" },
                        { key: "kmDepart", label: "Km départ" },
                        { key: "kmArrivee", label: "Km arrivée" },
                        { key: "carburant", label: "Carburant (L)" },
                        { key: "date", label: "Date", render: t => t.createdAt ? new Date(t.createdAt).toLocaleDateString() : "-" },
                    ]}/>
                </div>
            </div>
        </div>
