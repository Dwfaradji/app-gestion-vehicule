...
export default function DetailDepensesForVehicle({
  ...
  if (!vehicle) {
    return <div className="p-6 text-red-600 font-semibold">Aucun véhicule sélectionné.</div>;
  }
  ...
  const chartData = useMemo(() => {
    return [
      {
        categorie: "Mécanique",
        montant: filteredCategories.mecanique.reduce((sum, d) => sum + d.montant, 0),
      },
      {
        categorie: "Carrosserie",
        montant: filteredCategories.carrosserie.reduce((sum, d) => sum + d.montant, 0),
      },
      {
        categorie: "Révision",
        montant: filteredCategories.revision.reduce((sum, d) => sum + d.montant, 0),
      },
      {
        categorie: "Carburant",
        montant: filteredCategories.carburant.reduce((sum, d) => sum + d.montant, 0),
      },
      {
        categorie: "Contrôle technique",
        montant: filteredCategories.controleTechnique.reduce((sum, d) => sum + d.montant, 0),
      },
      {
        categorie: "Autres",
        montant: filteredCategories.autres.reduce((sum, d) => sum + d.montant, 0),
      },
    ];
  }, [filteredCategories]);
  ...
  const columns: Column<Depense>[] = [
    ...
    },
    {
      key: "montant",
      label: "Montant (€)",
      render: (d) => <span className="font-bold text-green-600">{d.montant} €</span>,
    },
  ];
  ...
  return (
    <div className="p-10 space-y-10">
      <h1 className="text-3xl font-bold mb-6">
        Détails des dépenses pour : {vehicle.constructeur} {vehicle.modele}
      </h1>
