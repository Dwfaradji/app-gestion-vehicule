...
import { Edit2, Save, Trash2, Car, Fuel, Users, MapPin, Droplet, CheckCircle2 } from "lucide-react";
...
export function DetailTrajetPage({ vehicules }: { vehicules: Vehicule[] }) {
    ...
    const {
        search, setSearch,
        selectedVehicule, setSelectedVehicule,
        selectedConducteur, setSelectedConducteur,
        dateStart, setDateStart,
        dateEnd, setDateEnd,
        heureStart, setHeureStart,
        heureEnd, setHeureEnd,
        disponibleOnly, setDisponibleOnly,
        resetFilters, filteredTrajets,
        infosManquantesOnly, setInfosManquantesOnly
    ...
    const springProps = useSpring({ number: dernierCarburant, from: { number: 0 }, config: { duration: 1000 } });

    // --- Ajouter / sauvegarder trajet ---
    const handleAjouterTrajet = () => setShowFormTrajet(true);

    const handleSaveTrajet = async (trajet: Trajet) => {
    ...
    const startEditing = (t: Trajet) => { setEditingRow(t.id); setEditValues({ ...t }); };

    const cancelEditing = () => {
        const current = vehiculeTrajets.find(t => t.id === editingRow);
        if (current && !current.conducteurId) setVehiculeTrajets(prev => prev.filter(t => t.id !== editingRow));
        setEditingRow(null);
        setEditValues({});
    };
    ...
    const saveEditing = async () => {
        ...
        try {
            if (trajets.find(t => t.id === current.id)) await updateTrajet({ ...current, anomalies: anomaliesFinales });
    ...
    // --- Définir colonnes dynamiquement ---
    const columns = [
        {
            key: "conducteurId",
            label: "Conducteur",
            render: t => editingRow === t.id
                ? <select
                ...
                : (conducteurs.find(c => c.id === t.conducteurId)?.prenom ?? "-") + " " + (conducteurs.find(c => c.id === t.conducteurId)?.nom ?? "")
        },
        { key: "destination", label: "Destination", render: t => editingRow === t.id ? <input type="text" value={editValues.destination ?? t.destination ?? ""} onChange={e => setEditValues({ ...editValues, destination: e.target.value })} className="w-full rounded border px-2 py-1"/> : t.destination ?? "-" },
        { key: "kmDepart", label: "Km départ", render: t => editingRow === t.id ? <input type="number" value={editValues.kmDepart ?? t.kmDepart ?? ""} onChange={e => setEditValues({ ...editValues, kmDepart: Number(e.target.value) })} className="w-20 rounded border px-2 py-1"/> : t.kmDepart ?? "-" },
        { key: "kmArrivee", label: "Km arrivée", render: t => editingRow === t.id ? <input type="number" value={editValues.kmArrivee ?? t.kmArrivee ?? ""} onChange={e => setEditValues({ ...editValues, kmArrivee: Number(e.target.value) })} className="w-20 rounded border px-2 py-1"/> : t.kmArrivee ?? "-" },
        { key: "heureDepart", label: "Heure départ", render: t => editingRow === t.id ? <input type="time" value={editValues.heureDepart ?? t.heureDepart ?? ""} onChange={e => setEditValues({ ...editValues, heureDepart: e.target.value })} className="w-24 rounded border px-2 py-1"/> : t.heureDepart ?? "-" },
        { key: "heureArrivee", label: "Heure arrivée", render: t => editingRow === t.id ? <input type="time" value={editValues.heureArrivee ?? t.heureArrivee ?? ""} onChange={e => setEditValues({ ...editValues, heureArrivee: e.target.value })} className="w-24 rounded border px-2 py-1"/> : t.heureArrivee ?? "-" },
        { key: "carburant", label: "Carburant", render: t => editingRow === t.id ? <input type="number" value={editValues.carburant ?? t.carburant ?? ""} onChange={e => setEditValues({ ...editValues, carburant: Number(e.target.value) })} className="w-20 rounded border px-2 py-1"/> : t.carburant ?? "-" },
        { key: "anomalies", label: "Anomalies", render: row => <AnomaliesCell anomalies={row.anomalies} isEditing={editValues.id === row.id}/> },
        {
            key: "actions",
            label: "Actions",
            render: row => {
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            {vehicule && (
                <div className="bg-gradient-to-r from-blue-50 via-blue-100 to-blue-50 p-8 rounded-3xl shadow-2xl border border-gray-200 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    <div className="flex flex-col gap-6">
                        <h2 className="text-3xl md:text-4xl font-extrabold text-blue-900 tracking-wide">{vehicule.type} - {vehicule.modele}</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-3 gap-4">
                            {[
                                ...
                                { label: "Carburant", value: dernierCarburant, icon: Droplet, color: "red", isAnimated: true },
                                { label: "Statut", value: vehiculeTrajets.length === 0 ? "Disponible" : "En usage", icon: CheckCircle2, color: vehiculeTrajets.length === 0 ? "green" : "yellow" }
                            ].map((item, idx) => {
                                const Icon = item.icon;
                                return (
                                    <div key={idx} className={`bg-${item.color}-50 p-4 rounded-xl shadow-md flex items-center gap-3`}>
                                        <Icon className={`text-${item.color}-600`} size={24} />
                                        <div className="flex flex-col">
                                            <span className="text-gray-500 text-xs font-medium">{item.label}</span>
                                            {item.isAnimated
                                                ? <animated.span>{springProps.number.to(n => `${Math.round(n)}%`)}</animated.span>
            ...
            <div className="mb-4 flex justify-end">
                <button onClick={handleAjouterTrajet} className="bg-green-600 text-white px-5 py-2 rounded-xl hover:bg-green-700">Ajouter un trajet</button>
            </div>

            {/* Formulaire */}
            <AnimatePresence>
                {showFormTrajet && (
                    <motion.div key="form-trajet" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="mb-6">
                        <FormulaireTrajet
                            ...
                            className="space-y-4"
                            inputClassName="max-w-sm sm:w-1/2"
                        />
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <SearchBarAdvanced
                vehicules={vehicules}
                conducteurs={conducteurs}
                trajets={vehiculeTrajets}
                search={search} setSearch={setSearch}
                selectedVehicule={selectedVehicule} setSelectedVehicule={setSelectedVehicule}
                selectedConducteur={selectedConducteur} setSelectedConducteur={setSelectedConducteur}
                dateStart={dateStart} setDateStart={setDateStart}
                dateEnd={dateEnd} setDateEnd={setDateEnd}
                heureStart={heureStart} setHeureStart={setHeureStart}
                heureEnd={heureEnd} setHeureEnd={setHeureEnd}
                disponibleOnly={disponibleOnly} setDisponibleOnly={setDisponibleOnly}
                infosManquantesOnly={infosManquantesOnly} setInfosManquantesOnly={setInfosManquantesOnly}
