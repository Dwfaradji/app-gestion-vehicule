    ...
    <AnimatePresence>
      {isLoading && (
        <motion.div
          ...
          className={clsx(
            fullscreen
              ? "fixed inset-0 z-50 bg-gray-50 dark:bg-gray-900 flex items-center justify-center"
              : "",
            "transition-all",
          )}
        >
          ...
          >
            ...
            {skeleton === "list" && (
              <ul className="w-full max-w-lg space-y-3">
                {[...Array(5)].map((_, i) => (
                  <li key={i} className="h-10 rounded bg-gray-200 dark:bg-gray-800 animate-pulse" />
                ))}
              </ul>
            )}
