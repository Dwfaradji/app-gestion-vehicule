...
const AnimatedDigit = ({ digit }: { digit: number }) => {
  ...
  return (
    <div className="overflow-hidden w-6 h-6 relative bg-gray-900 rounded-md shadow-inner flex items-start">
      ...
      >
        {[...Array(10).keys()].map((n) => (
          <p key={n} style={{ height: chiffreHeight }}>
            {n}
          </p>
        ))}
      </animated.div>
...
const CarteInfosVehicule = ({ vehicule }: { vehicule: Vehicule }) => {
  ...
  const [statut, setStatut] = useState<VehiculeStatus>(vehicule.statut ?? "Disponible");
  ...
  return (
      ...
      <div className="flex-1 rounded-2xl bg-white shadow-md p-6 border border-gray-100 hover:shadow-lg transition-all duration-200">
        ...
        <div className="space-y-3 text-gray-700">
          <div className="space-y-3 text-gray-700">
            ...
            <p>
              <strong>Constructeur :</strong> {vehicule.constructeur}
            </p>
            <p>
    ...
    `}
      ...
      <div className="flex-1 rounded-2xl bg-white shadow-md p-6 border border-gray-100 hover:shadow-lg transition-all duration-200">
        ...
        <div className="space-y-3 text-gray-700">
          <p className="flex items-center gap-2">
            <FaCogs className="text-gray-500" /> <strong>Motorisation :</strong>{" "}
            {vehicule.motorisation}
          </p>
          <p className="flex items-center gap-2">
            <FaGasPump className="text-gray-500" /> <strong>Energie :</strong> {vehicule.energie}
          </p>
          <p className="flex items-center gap-2">
            <FaUsers className="text-gray-500" /> <strong>Places :</strong> {vehicule.places}
          </p>
          <p className="flex items-center gap-2">
            <FaRoad className="text-gray-500" /> <strong>VIM :</strong> {vehicule.vim}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 rounded-2xl bg-white shadow-md p-6 border border-gray-100 hover:shadow-lg transition-all duration-200">
        ...
        <div className="space-y-3 text-gray-700">
          <p className="flex items-center gap-2">
            <FaCalendarAlt className="text-gray-500" /> <strong>Prochaine Révision :</strong>{" "}
            {formatDate(vehicule.prochaineRevision)}
          </p>
          <p className="flex items-center gap-2">
            <FaCalendarAlt className="text-gray-500" /> <strong>Date d&#39;entretien :</strong>{" "}
            {formatDate(vehicule.dateEntretien)}
          </p>
        </div>
      </div>
    </div>
