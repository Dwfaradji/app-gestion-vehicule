...
export default function SearchBarWithSuggestions({
    ...
    return (
        <div ref={containerRef} className="relative w-full max-w-5xl mx-auto mb-6">
            <div className={`flex items-center border rounded-xl px-4 py-2 shadow-sm transition-all duration-200 ${isFocused ? "border-blue-500 ring-2 ring-blue-200" : "border-gray-300"} bg-white`}>
                ...
                />
                {search && (
                    <button onClick={clearSearch} className="p-1 rounded-full hover:bg-gray-100 transition" title="Effacer">
                    ...
                    >
                        {suggestions.map(s => {
                            ...
                            return (
                                ...
                                >
                                    <span>{vehiculeT?.modele} ({vehiculeT?.immat}) - {s.destination}</span>
                                    <span className="text-gray-500">{conducteur ? `${conducteur.prenom} ${conducteur.nom}` : "-"}</span>
                                </div>
                            );
                        })}
                    </motion.div>
