...
test.describe("Conducteurs - FULL E2E UI", () => {
  ...
  test("Créer, modifier et supprimer un conducteur via UI puis vérifier en base", async ({
    page,
  }) => {
    ...
    // Vérifier en base
    const afterDelete = await prisma.conducteur.findUnique({ where: { id: Number(createdId) } });
